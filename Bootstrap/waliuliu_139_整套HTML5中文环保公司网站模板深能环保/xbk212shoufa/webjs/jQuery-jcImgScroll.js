		...
		return this.each(function() {
			var _SELF = $(this),
				...
				object = function(length){
					...
					// };
					this.arrow = function(l,r,w,h,x,y){
						return "<em class=\"sPrev\" id=\"sPrev\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:0px;\"></em><em class=\"sNext\" id=\"sNext\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;right:0px;top:"+y+"px;\"></em>";
					};
			...
			$prev.click(function(){
				if(_IDX > 0) {
					...
					$('.newhonorTit').html($('.jcImgScroll').find('li').eq(_IDX).find('a').attr('title'));
			...
			$next.click(function(){
				if(_IDX < imgLength-1) {
					_IDX = _IDX + 1;
					imgScroll(_IDX,_arrData,$dd);
					$('.newhonorTit').html($('.jcImgScroll').find('a').eq(_IDX).attr('title'));
				};
				return false;
			});
			$dd.click(function(){
			...
			if(options.title){
				...
				$(document).on("mouseover","li.select",_SELF,function(){
